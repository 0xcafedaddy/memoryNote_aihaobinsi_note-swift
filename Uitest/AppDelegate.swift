...
@UIApplicationMain
class AppDelegate: UIResponder, UIApplicationDelegate ,SFSwiftNotificationProtocol{
    ...
    func application(application: UIApplication, performFetchWithCompletionHandler completionHandler: (UIBackgroundFetchResult) -> Void) {
        //此处显然应该异步操作，偷懒而已。
        var shouldNotificationList  =  NoteNotificationDbManger().listShouldNotify()
        var notifyFrame = CGRectMake(0, 20, CGRectGetMaxX(self.window!.frame), 50)
        if let notification  =  shouldNotificationList.first{
    ...
    func didTapNotification(noteId:Int64){
        var noteDetailViewCrontroller = NoteDetailViewController()
        noteDetailViewCrontroller.noteId = noteId
        self.window?.rootViewController?.presentViewController(noteDetailViewCrontroller, animated: true, completion: nil)
    }
...
//        let modelURL = NSBundle.mainBundle().URLForResource("Uitest", withExtension: "momd")!
...
//        let url = self.applicationDocumentsDirectory.URLByAppendingPathComponent("Uitest.sqlite")
