...
class SFSwiftNotification: UIView, UICollisionBehaviorDelegate, UIDynamicAnimatorDelegate {
    ...
    init(frame: CGRect, title: String?, noteId:Int64?, animationType:AnimationType, direction:Direction, delegate: SFSwiftNotificationProtocol?) {
        super.init(frame: frame)
    ...
    func invokeTapAction() {
        if let _noteid = self.noteId{
            self.delegate!.didTapNotification(_noteid)
            self.canNotify = true
        }else{
